import Link from "next/link";
import React from "react";
import SignInButton from "./SignInButton";
import { getAuthSession } from "@/lib/auth";
import UserAccountNav from "./UserAccountNav";

type Props = {};

const Navbar = async (props: Props) => {
  const session = await getAuthSession();
  return (
    <nav className="fixed inset-x-0 top-0 bg-white dark:bg-gray-950 z-[10] h-fit border-b border-zinc-300 py-2">
      <div className="flex items-center justify-center h-full gap-2 px-8 mx-auto sm:justify-between max-w-7xl">
        <Link href="/" className="items-center hidden gap-2 sm:flex">
          <p className="rounded-lg px-2 py-1 text-xl font-bold text-customGreen transition-all hover:-translate-y-[2px] md:block dark:border-white">
            STREAM
          </p>
        </Link>

        {/* Conditional Rendering for Menu Items */}
        <div className="flex items-center space-x-6">
          {/* Show Home and Sign In before sign in */}
          {!session?.user && (
            <>
              <Link href="/" className="mr-3">
                Home
              </Link>
              <SignInButton />
            </>
          )}

          {/* Show Dashboard, Create, and Profile after sign in */}
          {session?.user && (
            <>
              <Link href="/gallery" className="mr-3">
                Dashboard
              </Link>
              <Link href="/create" className="mr-3">
                Create
              </Link>
              <UserAccountNav user={session.user} />
            </>
          )}
        </div>
      </div>
    </nav>
  );
};

export default Navbar;
